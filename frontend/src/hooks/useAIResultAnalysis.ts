...
const generateReportFromMapping = (
  ...
  /*
  ========================================================
  🔁 Extract mapped columns from fieldMappings
  ========================================================
  */
  ...
  */
  ...
  rows.forEach(r => {
    const name = r[studentNameColumn];
    if (name && name !== "-") {
      studentSet.add(name);
    }
  });
  const totalStudents = studentSet.size || rows.length;
  ...
  */
  ...
  // A student is considered passed if they passed ALL their subjects
  const studentOverallPass = new Map<string, boolean>();

  rows.forEach(r => {
    const studentName = r[studentNameColumn];
    if (!studentName || studentName === "-") return;

    const isPass = passValues.includes(r[criteriaColumn]);

    if (studentOverallPass.has(studentName)) {
      studentOverallPass.set(studentName, Boolean(studentOverallPass.get(studentName)) && isPass);
    } else {
      studentOverallPass.set(studentName, isPass);
    }
  });

  let totalPass = 0;
  studentOverallPass.forEach(passed => {
    if (passed) totalPass++;
  });
  ...
  */
  ...
  rows.forEach(r => {
    ...
    const rankVal = parseFloat(r[rankColumn] || "0");
    if (!isNaN(rankVal)) {
        // Keep the max rank if already exists (usually SGPA is same across rows of same student)
        studentRankMap.set(studentName, Math.max(rankVal, studentRankMap.get(studentName) || 0));
    }
  });

  const sortedStudents = Array.from(studentRankMap.entries()).sort((a, b) => b[1] - a[1]);

  const toppers: Topper[] = sortedStudents.slice(0, 3).map((s, idx) => ({
    rank: idx + 1,
    name: s[0],
    sgpa: s[1]
  }));
